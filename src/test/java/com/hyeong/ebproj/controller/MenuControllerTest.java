...
/* 가상의 요청(request)를 테스트 하기 위한 Mock객체 테스트용 설정(요청, 전송, 응답) */
...
class MenuControllerTest {
    ...
    public void findMenuCodeTest() throws Exception {
        mockMvc.perform(get("/menus/4"))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON)) //결과가 json 형태인지
                // menuName 컬럼이있을경우 그 값이 갈릭미역파르페 인지 확인
                .andExpect(jsonPath("$.menuName").value("갈릭미역파르페"))
                .andDo(print()); // 요청과 응답에대해 출력
    }
